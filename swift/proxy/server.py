...
class Application(object):
    ...
    def __init__(self, conf, memcache=None, logger=None, account_ring=None,
        ...
        self.superadmin = conf.get('swift_superadmin', 'admin')
    ...
    def handle_request(self, req):
        ...
        try:
            ...
            if 'swift.authorize' in req.environ:
                ...
                self.logger.debug("user role is :" + req.environ['HTTP_X_ROLE'])
                #add by garvenshen, 
                #make super-admin can get through the auth
                #to get all account info.(garvenshen)
                if(req.headers['X-User-Name']==self.superadmin):
                    self.logger.debug("a superadmin req.") 
                    resp = None
                else :
                    #if not the super-admin, use the defaut auth.(garvenshen)
